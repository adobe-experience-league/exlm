...
export default async function decorate(block) {
  ...
  const marqueeDOM = document.createRange().createContextualFragment(`
    ...
    <div class='marquee-background'>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1562 571.212">
        <path fill="${backgroundHexcode.innerHTML === 'true' ? bgColor : '#fff'}" d="M0 1.212h1562v570H0z" data-name="Rectangle 1"></path>
  ...
  const isVideoLinkType = firstCtaLinkType?.textContent?.trim() === 'video' || secondCtaLinkType?.textContent?.trim() === 'video';
  const isSigninLinkType = firstCtaLinkType?.textContent?.trim() === 'signin' || secondCtaLinkType?.textContent?.trim() === 'signin';
