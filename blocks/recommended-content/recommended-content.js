...
export default async function decorate(block) {
  ...
  if (targetSupport) {
    ...
    handleTargetEvent(targetCriteriaId).then((data) => {
      ...
      if (targetSupport) {
        headingElement.classList.add('recommened-target-header');
      }
    });
  }
  ...
  const fetchDataAndRenderBlock = async (optionType, renderCards = true) => {
    ...
    Promise.all(cardPromises)
      .then((finalPromise) => {
        const dontRenderCards = finalPromise.some((data) => data?.payloadConfig?.renderCards === false);
        if (saveCardResponse) {
          const renderedModels = finalPromise.reduce((acc, curr) => {
