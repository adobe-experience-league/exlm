...
(function () {
  ...
  var createHook = function createHook(componentRegistry, hookName) {
    return function () {
      ...
      return Promise.all(
        componentRegistry.getLifecycleCallbacks(hookName).map(function (callback) {
  ...
  var isSupportedAnchorElement = function isSupportedAnchorElement(element) {
    if (
      ...
      (!element.onclick || !element.protocol || element.protocol.toLowerCase().indexOf('javascript') < 0)
    ) {
      return true;
    }
  ...
  var getLinkRegion = function (node) {
    ...
    while (linkParentNode) {
      regionName = truncateWhiteSpace(
        linkParentNode.id || getAriaRegionLabel(linkParentNode) || getSectionNodeName(linkParentNode),
      );
  ...
  var getBrowser = memoize(function (window) {
    ...
    return matchUserAgent(
      ...
      _defineProperty(_matchUserAgent, CHROME, /(?!Chrom.*OPR)Chrom(?:e|ium)\/([0-9\.]+)(:?\s|$)/),
      _defineProperty(_matchUserAgent, FIREFOX, /Firefox\/([0-9\.]+)(?:\s|$)/),
      _defineProperty(_matchUserAgent, IE, /Trident\/7\.0.*rv\:([0-9\.]+).*\).*Gecko$/),
  ...
  var groupDecisions = function groupDecisions(unprocessedDecisions) {
    ...
    return {
      ...
      nonAutoRenderableDecisions: [].concat(
        ...
        _toConsumableArray(decisionsGroupedByRenderableSchemas.unmatchedDecisions),
  ...
  var buildAllOnInstanceConfiguredExtraParams = function buildAllOnInstanceConfiguredExtraParams(
    config,
    logger,
    componentCreators,
  ) {
