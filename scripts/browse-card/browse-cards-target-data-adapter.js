...
const BrowseCardsTargetDataAdapter = (() => {
  ...
  const mapResultsToCardsDataModel = (data) => {
    ...
    // Extract course-related fields (support both naming conventions)
    const level = data?.el_level || data?.level || '';
    const duration = data?.el_course_duration || data?.courseDuration || data?.duration || '';
    const moduleCount = data?.el_course_module_count || data?.courseModuleCount || data?.moduleCount || '';
